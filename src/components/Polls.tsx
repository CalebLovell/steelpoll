		...
		<li key={poll.title} className='col-span-1 p-4 space-y-3 rounded-lg shadow bg-brand-secondary'>
			<div className='flex items-center justify-between'>
				...
				<div className={`px-2 rounded-full flex items-center ${poll?.options?.protected ? `bg-blue-100` : `bg-green-100`}`}>
					<p className={`text-sm font-medium ${poll?.options?.protected ? `text-blue-800` : `text-green-800`}`}>
			...
			<div className='flex justify-between'>
				<a href={`poll/${poll?.id}`} className='flex items-center justify-center text-sm font-medium'>
					<ArchiveIcon className='w-5 h-5 text-brand-primary' aria-hidden='true' />
					<span className='ml-3 text-brand-primary'>Vote Now</span>
				</a>
				<a href={`poll/${poll?.id}/results`} className='flex items-center justify-center text-sm font-medium'>
					<ChartPieIcon className='w-5 h-5 text-brand-primary' aria-hidden='true' />
					<span className='ml-3 text-brand-primary'>View Results</span>
				</a>
			</div>
		</li>
	);
