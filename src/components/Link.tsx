...
export const Link = ({ href, label, variant }: Props) => {
	const getClassName = () => {
		switch (variant) {
			...
			case `button`:
				return `p-2 text-center box-border font-medium transition duration-150 ease-in-out bg-brand-primary rounded-md focus-brand-without-border hover-brand lg:px-4 text-white`;
			case `mobile`:
